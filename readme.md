...
- If you prefer not using integrated graphics, set `moeDiscreteOnly=1` environment variable, `nvidia.NVreg_EnableGpuFirmware=0` kernel cmdline and add `nvidia nvidia_modeset nvidia_drm` to initramfs modules. It is also recommended to enable `nvidia-suspend.service`, `nvidia-resume.service` and set kernel parameter `nvidia.NVreg_TemporaryFilePath=/var/tmp`.
