...
- x86-64-v3 CPU.
- Arch Linux's vanilla kernel with GNOME Wayland only, Plasma flavor can be built by checking out to branch `plasma` in moeOS-Package and set an environment variable `moePreferDE=KDE` (it is buggy).
